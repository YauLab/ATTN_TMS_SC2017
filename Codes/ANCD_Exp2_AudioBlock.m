...
%% Experiment 2 (intensity discrimination) AUDIO block: code for analysis

% Upload the mat file 'TMS_ATintXX_XXXX_Audio.mat'

% allocating space for the following column vectors

tmp_RPmt=zeros(length(AUDIODATA.resp),1);
RPmt=zeros(length(tmp_RPmt),1);
resp=zeros(length(tmp_RPmt),1);
Acc_resp=zeros(length(tmp_RPmt),1);

%Transforming the content of AUDIODATA.resp field into 1/2 for analysis
%purposes

for i= 1:length(tmp_RPmt)
    if AUDIODATA.resp(i)=='z'
        RPmt(i)=1;
    else
        RPmt(i)=2;
    end
...
infoRP=[AUDIODATA.trialinfo, RPmt];
...
mean_acc_audio_block = mean(Acc_resp);
